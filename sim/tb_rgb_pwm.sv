...
module tb_rgb_pwm;
  ...
  lab3_rgb_pwm #(.CLK_HZ(1000), .PWM_HZ(100), .LEVELS(10), .DEBOUNCE_CYCLES(2)) dut
    (.clk_50mhz(clk_50mhz), .rst_p(rst_p), .button_r(button_r), .button_g(button_g),
     .button_b(button_b), .led_r(led_r), .led_g(led_g), .led_b(led_b));
  ...
  task measure(input integer er, input integer eg, input integer eb);
    begin
      ...
      for (i = 0; i < 10; i = i + 1) begin
        @(posedge clk_50mhz); #1; hr += led_r[0]; hg += led_g[0]; hb += led_b[0];
      end
